#!/usr/bin/env python3
"""Cut the Leadership page's portraits from the photographs the school supplied.

The supplied photographs are full-length, taken on the lawns, and 1000-1400px
wide. The page used to hand them to the browser whole: a 587 KB file for a
64px disc beside the Principal's message, and a card that cropped two of the
three at the knees rather than the face. This cuts every portrait to the same
two shapes instead, so the three people with a photograph read as one set:

    leadership/<slug>.jpg        4:5, head and shoulders, for the roster
    leadership/<slug>-face.jpg   square, the face alone, for the messages

Only people whose photograph the school supplied under their name appear
here. The sources are the files the owner's design artifact named
(tools/media.tsv), kept in assets/img because tools/artifact-reference.html
still shows them. Nobody is given a photograph that was not supplied as theirs.
...
# placed by eye so that the eyes sit about a third of the way down.
CUTS = {
...
# The Founder has no roster entry — he is not on the Board — only a face
# beside his message, from the portrait the Founder page already uses.
FACES = {
